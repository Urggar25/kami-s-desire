# TODO: Translation updated at 2026-08-07 13:14

translate portuguese strings:

    # game/screens.rpy:203
    old "DECISION"
    new "DECISÃO"

    # game/screens.rpy:314
    old "Nouvelle partie"
    new "Novo jogo"

    # game/screens.rpy:316
    old "Historique"
    new "Histórico"

    # game/screens.rpy:317
    old "Sauvegarde"
    new "Salvar"

    # game/screens.rpy:319
    old "Charger"
    new "Carregar"

    # game/screens.rpy:320
    old "Préférences"
    new "Preferências"

    # game/screens.rpy:321
    old "Succès"
    new "Conquistas"

    # game/screens.rpy:324
    old "Roadmap"
    new "Roadmap"

    # game/screens.rpy:325
    old "Codex"
    new "Códice"

    # game/screens.rpy:326
    old "Codes promo"
    new "Códigos promocionais"

    # game/screens.rpy:328
    old "Profils"
    new "Perfis"

    # game/screens.rpy:333
    old "Fin de la rediffusion"
    new "Encerrar replay"

    # game/screens.rpy:335
    old "Menu principal"
    new "Menu principal"

    # game/screens.rpy:337
    old "À propos"
    new "Sobre"

    # game/screens.rpy:340
    old "Aide"
    new "Ajuda"

    # game/screens.rpy:343
    old "Quitter"
    new "Sair"

    # game/screens.rpy:489
    old "Retour"
    new "Voltar"

    # game/screens.rpy:577
    old "Version [config.version!t]\n"
    new "Versão [config.version!t]\n"

    # game/screens.rpy:582
    old "Conçu avec {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new "Criado com {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"

    # game/screens.rpy:613
    old "Page {}"
    new "Página {}"

    # game/screens.rpy:613
    old "Sauvegardes automatiques"
    new "Salvamentos automáticos"

    # game/screens.rpy:613
    old "Sauvegardes rapides"
    new "Salvamentos rápidos"

    # game/screens.rpy:646
    old "{#file_time}%A %d %B %Y, %H:%M"
    new "{#file_time}%A, %d de %B de %Y, %H:%M"

    # game/screens.rpy:646
    old "emplacement vide"
    new "slot vazio"

    # game/screens.rpy:663
    old "<"
    new "<"

    # game/screens.rpy:667
    old "{#auto_page}A"
    new "{#auto_page}A"

    # game/screens.rpy:670
    old "{#quick_page}Q"
    new "{#quick_page}Q"

    # game/screens.rpy:675
    old ">"
    new ">"

    # game/screens.rpy:680
    old "Uploader Sync"
    new "Enviar Sync"

    # game/screens.rpy:684
    old "Télécharger Sync"
    new "Baixar Sync"

    # game/screens.rpy:755
    old "Affichage"
    new "Exibição"

    # game/screens.rpy:756
    old "Fenêtre"
    new "Janela"

    # game/screens.rpy:757
    old "Plein écran"
    new "Tela cheia"

    # game/screens.rpy:761
    old "Avance rapide"
    new "Pular"

    # game/screens.rpy:762
    old "Texte non lu"
    new "Texto não lido"

    # game/screens.rpy:763
    old "Après les choix"
    new "Após as escolhas"

    # game/screens.rpy:764
    old "Transitions"
    new "Transições"

    # game/screens.rpy:768
    old "Langage"
    new "Idioma"

    # game/screens.rpy:769
    old "Français"
    new "Francês"

    # game/screens.rpy:770
    old "Anglais"
    new "Inglês"

    # game/screens.rpy:779
    old "Vitesse du texte"
    new "Velocidade do texto"

    # game/screens.rpy:782
    old "Avance automatique"
    new "Avanço automático"

    # game/screens.rpy:787
    old "Volume de la musique"
    new "Volume da música"

    # game/screens.rpy:792
    old "Volume des sons"
    new "Volume dos efeitos sonoros"

    # game/screens.rpy:796
    old "Test"
    new "Teste"

    # game/screens.rpy:799
    old "Volume des voix"
    new "Volume das vozes"

    # game/screens.rpy:807
    old "Couper tous les sons"
    new "Silenciar todos os sons"

    # game/screens.rpy:933
    old "L'historique des dialogues est vide."
    new "O histórico de diálogos está vazio."

    # game/screens.rpy:997
    old "Clavier"
    new "Teclado"

    # game/screens.rpy:998
    old "Souris"
    new "Mouse"

    # game/screens.rpy:1001
    old "Manette"
    new "Controle"

    # game/screens.rpy:1013
    old "Entrée"
    new "Enter"

    # game/screens.rpy:1014
    old "Avance dans les dialogues et active l'interface."
    new "Avança os diálogos e ativa a interface."

    # game/screens.rpy:1016
    old "Espace"
    new "Espaço"

    # game/screens.rpy:1017
    old "Avance dans les dialogues sans effectuer de choix."
    new "Avança os diálogos sem fazer escolhas."

    # game/screens.rpy:1019
    old "Flèches directionnelles"
    new "Setas direcionais"

    # game/screens.rpy:1020
    old "Permet de se déplacer dans l'interface."
    new "Permitem navegar pela interface."

    # game/screens.rpy:1022
    old "Echap."
    new "Esc"

    # game/screens.rpy:1023
    old "Ouvre le menu du jeu."
    new "Abre o menu do jogo."

    # game/screens.rpy:1025
    old "Ctrl"
    new "Ctrl"

    # game/screens.rpy:1026
    old "Fait défiler les dialogues tant que la touche est pressée."
    new "Pula os diálogos enquanto a tecla estiver pressionada."

    # game/screens.rpy:1028
    old "Tab"
    new "Tab"

    # game/screens.rpy:1029
    old "Active ou désactive les «sauts des dialogues»."
    new "Ativa ou desativa o modo de pular diálogos."

    # game/screens.rpy:1031
    old "Page Haut"
    new "Page Up"

    # game/screens.rpy:1032
    old "Retourne au précédent dialogue."
    new "Volta ao diálogo anterior."

    # game/screens.rpy:1034
    old "Page Bas"
    new "Page Down"

    # game/screens.rpy:1035
    old "Avance jusqu'au prochain dialogue."
    new "Avança até o próximo diálogo."

    # game/screens.rpy:1038
    old "Cache l'interface utilisateur."
    new "Oculta a interface do usuário."

    # game/screens.rpy:1041
    old "Prend une capture d'écran."
    new "Faz uma captura de tela."

    # game/screens.rpy:1044
    old "Active la {a=https://www.renpy.org/l/voicing}{size=24}vocalisation automatique{/size}{/a}."
    new "Ativa a {a=https://www.renpy.org/l/voicing}{size=24}leitura automática por voz{/size}{/a}."

    # game/screens.rpy:1047
    old "Ouvre le menu d'accessibilité."
    new "Abre o menu de acessibilidade."

    # game/screens.rpy:1052
    old "Bouton gauche"
    new "Botão esquerdo"

    # game/screens.rpy:1055
    old "Bouton central"
    new "Botão do meio"

    # game/screens.rpy:1058
    old "Bouton droit"
    new "Botão direito"

    # game/screens.rpy:1061
    old "Molette vers le haut"
    new "Roda para cima"

    # game/screens.rpy:1064
    old "Molette vers le bas"
    new "Roda para baixo"

    # game/screens.rpy:1070
    old "Stick gauche\nD-pad"
    new "Analógico esquerdo\nD-pad"

    # game/screens.rpy:1071
    old "Déplace le curseur dans les PnC, l'exploration, les menus et les mini-jeux."
    new "Move o cursor nas sequências PnC, na exploração, nos menus e nos minijogos."

    # game/screens.rpy:1073
    old "A/Bouton du bas"
    new "A/Botão inferior"

    # game/screens.rpy:1074
    old "Clique, maintient ou glisse avec le curseur."
    new "Clica, mantém pressionado ou arrasta com o cursor."

    # game/screens.rpy:1076
    old "X/Bouton gauche"
    new "X/Botão esquerdo"

    # game/screens.rpy:1077
    old "Effectue un clic secondaire quand une interface l'utilise."
    new "Executa um clique secundário quando a interface permite."

    # game/screens.rpy:1079
    old "R1"
    new "R1"

    # game/screens.rpy:1080
    old "Accélère le déplacement du curseur."
    new "Acelera o movimento do cursor."

    # game/screens.rpy:1082
    old "Gâchettes"
    new "Gatilhos"

    # game/screens.rpy:1083
    old "Fait défiler les listes et panneaux."
    new "Rola listas e painéis."

    # game/screens.rpy:1088
    old "D-pad"
    new "D-pad"

    # game/screens.rpy:1089
    old "Navigue aussi dans les interfaces Ren'Py classiques."
    new "Também permite navegar pelas interfaces clássicas do Ren'Py."

    # game/screens.rpy:1091
    old "Start, Guide, B/Bouton droit"
    new "Start, Guide, B/Botão direito"

    # game/screens.rpy:1094
    old "Y/Bouton du haut"
    new "Y/Botão superior"

    # game/screens.rpy:1096
    old "Calibrage"
    new "Calibração"

    # game/screens.rpy:1158
    old "Oui"
    new "Sim"

    # game/screens.rpy:1159
    old "Non"
    new "Não"

# TODO: Translation updated at 2026-08-16 12:24

translate portuguese strings:

    # game/screens.rpy:765
    old "Accessibilité"
    new ""

    # game/screens.rpy:766
    old "Animations de dialogue"
    new ""

# TODO: Translation updated at 2026-08-23 11:52

translate portuguese strings:

    # game/screens.rpy:398
    old "Créer une sauvegarde de votre progression actuelle."
    new ""

    # game/screens.rpy:398
    old "Reprendre l'histoire depuis une sauvegarde existante."
    new ""

    # game/screens.rpy:398
    old "Régler l'affichage, le texte, le son et les contrôles."
    new ""

    # game/screens.rpy:398
    old "Consulter les dossiers des représentants du Conclave."
    new ""

    # game/screens.rpy:398
    old "Voir les succès découverts et ceux encore verrouillés."
    new ""

    # game/screens.rpy:398
    old "Parcourir les jours, embranchements et choix du récit."
    new ""

    # game/screens.rpy:398
    old "Interrompre la partie et retourner au menu principal."
    new ""

    # game/screens.rpy:398
    old "Fermer complètement Kami's Desires."
    new ""

    # game/screens.rpy:465
    old "MENU SYSTÈME"
    new ""

    # game/screens.rpy:472
    old "PAUSE // ACCÈS LOCAL"
    new ""

    # game/screens.rpy:495
    old "SAUVEGARDER"
    new ""

    # game/screens.rpy:506
    old "CHARGER"
    new ""

    # game/screens.rpy:517
    old "OPTIONS / PRÉFÉRENCES"
    new ""

    # game/screens.rpy:528
    old "PROFILS"
    new ""

    # game/screens.rpy:550
    old "ROADMAP"
    new ""

    # game/screens.rpy:561
    old "MENU PRINCIPAL"
    new ""

    # game/screens.rpy:572
    old "QUITTER LE JEU"
    new ""

    # game/screens.rpy:582
    old "REPRENDRE  ·  ÉCHAP"
    new ""

    # game/screens.rpy:611
    old "Sauvegarder"
    new ""

    # game/screens.rpy:613
    old "Options / Préférences"
    new ""

    # game/screens.rpy:624
    old "Quitter le jeu"
    new ""

