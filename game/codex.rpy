...
init python:
    ...
    # ------------------------------------------------------------------
    _CODEX_DATA = {
        "harmonie": {"title": "Harmonie", "icon": "ic_building",
            "short": "District administratif établi sur l'ancienne capitale veyronne.",
            "assoc": ["archive"],
            "text": "Harmonie est un district principalement administratif et urbain, fondé sur l'ancienne capitale de l'Empire Veyron après la prise de pouvoir de Kami.\n\nLe district compte environ 650 000 habitants."},
        "archive": {"title": "Archive", "icon": "ic_antenna",
            "short": "District chargé de l'information et de la propagande du régime.",
            "assoc": ["harmonie"],
            "text": "Archive est un petit district administratif construit dans une immense tour qui comporterait, d'après la légende, mille étages.\n\nLe district contrôle aujourd'hui les flux d'informations et la propagande du régime de Kami. Il compte environ 22 500 habitants.\n\nAvant la chute de l'Empire Veyron, la Tour était un monument national. Archive est enclavée au sein d'Harmonie."},
...
init 3 python:
    ...
    _CODEX_INGAME_ENTRIES = set(["reglement_conclave", "harmonie", "archive", "complexe_c"])
...
init python:
    ...
    # ---- Liens intégrés aux dialogues ----
    def codex_dialogue_link(eid, label):
        if not codex_is_unlocked(eid):
            return label
        return "{a=codex:%s}{color=#F0A835}{b}%s{/b}{/color}{/a}" % (eid, label)
