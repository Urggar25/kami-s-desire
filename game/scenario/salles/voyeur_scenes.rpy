...
label temps_libre_salle_stockage_argument:
    ...
    if not got_argument_echanges_discrets:
        ...
        if "DAY2_VOTE_ARGUMENTS" in globals() and "DAY3_VOTE_ARGUMENTS" in globals() and "echanges_discrets" not in DAY2_VOTE_ARGUMENTS:
            $ DAY2_VOTE_ARGUMENTS["echanges_discrets"] = DAY3_VOTE_ARGUMENTS["echanges_discrets"]
        if "day2_vote_add_argument" in globals():
            $ day2_vote_add_argument("echanges_discrets")
        else:
            $ add_argument("?changes discrets d?j? actifs")
            if "unlock_dossier_arg" in globals():
                $ unlock_dossier_arg("echanges_discrets")
        show screen argument_unlock("?changes discrets d?j? actifs")
